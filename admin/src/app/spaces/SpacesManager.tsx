...
export default function SpaceManager({ spaces, allProducts }: { spaces: Space[], allProducts: { id: string, name: string, sku: string, images: string[] }[] }) {
  ...
  const handleDelete = (id: string, productCount: number) => {
    if (productCount > 0) { alert(`Cannot delete: ${productCount} product(s) are assigned to this space.`); return; }
    ...
    <div className="space-y-6">
      ...
      {showForm && (
        <div className="bg-surface border border-accent/30 p-8 space-y-6 shadow-2xl">
          <h3 className="font-serif text-[24px] text-primary">{editing ? 'Edit Space' : 'Create New Space'}</h3>
            ...
            <div className="flex gap-3 items-end bg-background/50 p-4 border border-border">
              <div className="flex-1">
                ...
                >
                  ...
                  {allProducts
                    .filter(p => !spaces.find(s => s.id === managingProducts.id)?.products.some(sp => sp.id === p.id))
                    .map(p => (
            ...
            <div className="table-responsive border border-border">
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-border/50">
                  {spaces.find(s => s.id === managingProducts.id)?.products.map(p => (
                  ...
                  {(!spaces.find(s => s.id === managingProducts.id)?.products.length) && (
            ...
            {spaces.map(s => (
              <tr key={s.id} className="border-b border-border hover:bg-surface-muted">
                ...
                <td className="px-6 py-4 text-right flex gap-4 justify-end">
                  <button onClick={() => openManage(s)} className="font-mono text-[9px] uppercase tracking-[0.1em] text-accent hover:text-white transition-colors">Products</button>
                  <button onClick={() => openEdit(s)} className="font-mono text-[9px] uppercase tracking-[0.1em] text-muted hover:text-white transition-colors">Edit</button>
                  <button onClick={() => handleDelete(s.id, s._count.products)} className="font-mono text-[9px] uppercase tracking-[0.1em] text-red-400/60 hover:text-red-400 transition-colors">Delete</button>
                </td>
              </tr>
            ))}
            {spaces.length === 0 && (
